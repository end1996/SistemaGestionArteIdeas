...
document.addEventListener('click', function(event) {
 ...
 // Lógica de filtrado de contratos (en la pestaña Contratos Generales)
 function aplicarFiltrosContratos() {
    // Obtener valores de los filtros
    const filtroTexto = document.querySelector('.filtros-section input[type="text"]').value.toLowerCase();
    ...
    tablas.forEach(tbody => {
        ...
        filas.forEach(fila => {
            // Cliente
            const cliente = fila.querySelector('.cliente-nombre')?.textContent.toLowerCase() || '';
            // Servicio
            const servicio = fila.children[1]?.textContent.toLowerCase() || '';
            // Estado (badge)
